...
func (err *GapApplyRefusal) Error() string {
	return fmt.Sprintf("%s: %s (strategy %s, generation %s, expected marker revision %d)",
		err.Stage, err.Status, err.Plan.StrategyID, err.StateGeneration, err.ExpectedRevision)
}
