...
// the established encoder with no error and no alert -- an action named
// "enter the terminal state" whose effect is to return to the start.
//
...
// at least one has not run its own shadow. Changing this default is what makes
// the new encoder answer in an environment that never proved it there.
//
