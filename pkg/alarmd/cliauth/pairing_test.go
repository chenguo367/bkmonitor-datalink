...
func TestTheRenewalCredentialRollsAndTheSpentOneIsGone(t *testing.T) {
	...
	if w := refresh(m, login.RefreshToken); w.Code != 401 || errorCode(t, w) != "renewal_expired_or_revoked" {
		t.Fatalf("a spent credential renewed again: %d %s", w.Code, w.Body)
	}
	...
	stats := m.Stats()
	if stats.Counts[CountRenewed] != 2 || stats.Counts[CountRenewalExpired] != 1 || stats.Counts[CountPairingsIssued] != 1 ||
