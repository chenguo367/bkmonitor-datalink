...
func deploymentSection(ctx context.Context, native http.Handler, ops map[string]Operation) map[string]any {
	...
	findings = append(findings, redisFindings(info)...)
...
// redisFindings is the two named findings on every Redis server's INFO: an
// eviction policy other than noeviction, and keys already evicted. A server
// whose INFO did not answer, or answered without the field, is its own
// finding: an unread policy is not a safe one.
func redisFindings(info any) []DeploymentFinding {
